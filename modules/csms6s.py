...
def flops_selective_scan_fn(B=1, L=256, C=768, N=16, with_D=True, with_Z=False, with_complex=False):
    """
    u: r(B C L)
    delta: r(B C L)
    A: r(C N)
...
def flops_selective_scan_ref(B=1, L=256, C=768, N=16, with_D=True, with_Z=False, with_Group=True, with_complex=False):
    """
    u: r(B C L)
    delta: r(B C L)
    A: r(C N)
    ...
    flops = 0
    flops += get_flops_einsum([[B, C, L], [C, N]], "bdl,dn->bdln")
    if with_Group:
        flops += get_flops_einsum([[B, C, L], [B, N, L], [B, C, L]], "bdl,bnl,bdl->bdln")
    else:
        flops += get_flops_einsum([[B, C, L], [B, C, N, L], [B, C, L]], "bdl,bdnl,bdl->bdln")
  
    in_for_flops = B * C * N   
    if with_Group:
        in_for_flops += get_flops_einsum([[B, C, N], [B, C, N]], "bdn,bdn->bd")
    else:
        in_for_flops += get_flops_einsum([[B, C, N], [B, N]], "bdn,bn->bd")
...
def selective_scan_flop_jit(inputs, outputs, backend="prefixsum", verbose=True):
    ...
    B, C, L = inputs[0].type().sizes()
    N = inputs[2].type().sizes()[1]
    flops = flops_fn(B=B, L=L, C=C, N=N, with_D=True, with_Z=False)
    return flops
